...
import
  logos_delivery/waku/[node/peer_manager, waku_core],
  logos_delivery/waku/waku_filter_v2/[common, client, subscriptions, protocol],
...
suite "Waku Filter - DOS protection":
  ...
  asyncSetup:
    ...
    wakuFilter = await newTestWakuFilter(
      serverSwitch, rateLimitSetting = Opt.some((3, 1000.milliseconds))
    )
